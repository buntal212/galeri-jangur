...
<template>
  <section id="lokasi" class="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
    <div class="mb-8 max-w-2xl">
      <p class="text-xs font-bold uppercase tracking-[0.2em] text-amber-500">Lokasi kami</p>
      <h2 class="mt-3 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">Lokasi Jangur Keramik Probolinggo</h2>
      <p class="mt-4 text-sm leading-6 text-slate-500">Kunjungi Jangur Keramik di Kecamatan Sumberasih, Kabupaten Probolinggo untuk melihat langsung koleksi keramik dan granit kami.</p>
    </div>
    <div class="grid overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xl shadow-slate-200/40 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <div v-else class="flex min-h-72 items-center justify-center bg-slate-100 p-8 text-center text-sm text-slate-500">{{ loading ? 'Memuat peta lokasi...' : 'Peta lokasi belum tersedia.' }}</div>
      <div class="flex flex-col justify-center p-7 sm:p-10">
        ...
        <p class="mt-4 text-sm leading-7 text-slate-500">Jalan Raya Jangur Barat Sungai Paser, Pacar, Jangur, Kecamatan Sumberasih, Kabupaten Probolinggo, Jawa Timur 67251.</p>
        <a v-if="directionUrl" :href="directionUrl" target="_blank" rel="noopener noreferrer" class="mt-7 inline-flex w-fit rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">Petunjuk Arah</a>
      </div>
    </div>
  </section>
</template>
