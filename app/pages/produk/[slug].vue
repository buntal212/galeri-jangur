...
const imageUrl = (path) => {
  ...
  return `${config.public.backendUrl || ''}${normalizedPath}`
}
...
const mainImageElement = ref(null)
const syncImageLoadingState = () => {
  if (mainImageElement.value?.complete) imageLoading.value = false
}
watch(mainImage, value => {
  imageLoading.value = Boolean(value)
  imageFailed.value = false
  nextTick(syncImageLoadingState)
}, { immediate: true })
onMounted(() => nextTick(syncImageLoadingState))
